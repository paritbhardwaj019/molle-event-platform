...
export async function GET(
  ...
  try {
    ...
    if (
      (userRole === "USER" && conversation.userId !== userId) ||
    ...
    // Get messages
    const messages = await db.userHostMessage.findMany({
      where: {
        conversationId,
      },
      include: {
    ...
    const updatedMessages = await db.userHostMessage.updateMany({
      where: {
        conversationId,
        senderId: {
          not: userId,
    ...
    if (updatedMessages.count > 0) {
      ...
      const conversation = await db.userHostConversation.findUnique({
        where: { id: conversationId },
        select: { userId: true, hostId: true },
      });
    ...
    console.error("Error fetching user-host messages:", error);
    return NextResponse.json(
