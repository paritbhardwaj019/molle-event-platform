...
import { updateInviteForm } from "@/lib/actions/invite-form";
...
interface EditInviteFormDialogProps {
  open: boolean;
...
export function EditInviteFormDialog({
  open,
  onOpenChange,
  form: inviteForm,
  onSuccess,
}: EditInviteFormDialogProps) {
  ...
  useEffect(() => {
    if (inviteForm && open) {
      form.reset({
        ...
        fields: inviteForm.fields.map((field) => ({
          name: field.name,
  ...
  const onSubmit = async (data: InviteFormData) => {
    setIsLoading(true);
    try {
      const result = await updateInviteForm({
        id: inviteForm.id,
        name: data.name,
      ...
      if (result.success) {
        toast.success("Invite form updated successfully");
        onOpenChange(false);
        onSuccess?.();
      } else {
        toast.error(result.error || "Failed to update invite form");
      }
    } catch (error) {
      toast.error("An error occurred while updating the invite form");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Invite Form</DialogTitle>
          <DialogDescription>
            Update your custom invite form fields and settings.
          </DialogDescription>
        </DialogHeader>
